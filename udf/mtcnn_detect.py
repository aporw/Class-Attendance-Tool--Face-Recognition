...
# Superclass object explanation:
# In short, it sets free magical ponies.
# In long, Python 2.2 and earlier used "old style classes". 
# They were a particular implementation of classes, and they had a few limitations 
# (for example, you couldn't subclass builtin types). 
# The fix for this was to create a new style of class. 
# But, doing this would involve some backwards-incompatible changes. 
# So, to make sure that code which is written for old style classes will still work, 
# the object class was created to act as a superclass for all new-style classes. 
...
class MTCNNDetect(object):
    def __init__(self, face_rec_graph, model_path = "models", threshold = [0.6, 0.7, 0.7], factor = 0.709, scale_factor = 1):
        '''
        :param face_rec_sess: FaceRecSession, seems like have changed to face_rec_graph
        ...
        # Load MTCNN model to the specific graph.
        # save np.load
        np_load_old = np.load
        ...
        with face_rec_graph.graph.as_default():
            ...
            if not model_path:
                ...
                # os.path.realpath(__file__)是脚本所在的绝对路径， 确保可以得到正确路径， load model。
    ...
    def detect_face(self, img, minsize):
        ...
        # if scale factor > 1, we need to 
        if(self.scale_factor > 1):
            # resize: 使用cv2.resize时，参数输入是 宽×高×通道 ，与以往操作不同，需要注意。
            img = cv2.resize(img,(int(len(img[0])/self.scale_factor), int(len(img)/self.scale_factor)))
        factor_count = 0
        total_boxes = np.empty((0, 9)) # should be an empty array: []
        points = []
        h = img.shape[0] # a scalar?
        w = img.shape[1] # a scalar?
        # Return the minimum of an array or minimum along an axis.
        minl = np.amin([h, w]) # should be the smaller one btw h and w?
        m = 12.0 / minsize # setted value
        minl = minl * m
        # creat scale pyramid
        scales = []
        while minl >= 12:
